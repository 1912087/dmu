...
<link rel="stylesheet"
	href="http://localhost:9000/css/tiket.css">
...
$(document).ready(function(){
	...
	function ajax_orderby_list(day,rpage){
			$.ajax({
				url : 'orderby_event_ajaxlist?day='+day+"&rpage="+rpage, 
				type : "get" ,
				cache : false,
				...
				success : function(data){
					let dataset = JSON.parse(data);
						...
						for(aj of dataset.list){
							...
							output +=	"<a data-v-1e8092ec='' href='http://localhost:9000/dmu/adminticketlist_content?did="+aj.did+"' class='thumb'>"
							output +=	"<img data-v-2fed1a9a='' data-v-1e8092ec='' src='http://localhost:9000/upload/"+aj.dsfile+"'>"
							output +=	"</a>"
							output +=	"<a data-v-1e8092ec='' href='http://localhost:9000/dmu/adminticketlist_content?did="+aj.did+"' class='title'>"+aj.dtitle+"</a>" 
		...
		$('#more_button').click(function(){
			...
			$(location).attr('href', "http://localhost:9000/dmu/adminevent_list?rpage="+${rpage+1});
			});
			 var rpage = "${rpage}";
			if(rpage != 1){
...
<body>
														...
														<c:forEach var="vo" items="${list}">
															<ul data-v-1e8092ec="" class="list_move">
																<li data-v-1e8092ec=""><c:if
																		test="${vo.dsfile != null }">
																		<a data-v-1e8092ec=""
																			href="/adminticketlist_content/${vo.did}"
																			class="thumb"> <img data-v-2fed1a9a=""
																			data-v-1e8092ec=""
																			src="http://localhost:9000/upload/${vo.dsfile }"></a>
	...
	<iframe src="/footer" width="100%" heig"490px" scrolling="no"
